...
(define (domain multilevel-blocks)
  ...
  (:predicates
    ...
    ;; --- derived ---
    ;; Cell ?c is supported: nothing directly below it (table-level), OR
    ;; the cell directly below is non-empty.  Replaces the inline
    ;; (or (not (exists ...)) (exists ...)) support-check that used to live
    ;; in every put-* precondition.  Inline disjunctive action preconditions
    ;; make pymimir split each put-* into multiple action schemas with
    ;; witness parameters of differing arity, which breaks the GNN encoder's
    ;; one-update-module-per-name assumption.  As a derived predicate the
    ;; existential stays out of the action schemas, so each put-* keeps a
    ;; single arity.  (puttable is kept as-is — it's the static put-target
    ;; pre-filter and does not introduce witness parameters.)
    (supported ?c - cell)
  )

  (:derived (supported ?c - cell)
    (or
  ...
  (:action put-long-x
    ...
    :precondition (and
      ...
      ;; CENTRE c2 supported alone, OR both ENDS c1 and c3 supported.
      ;; Both branches reference only the unary (supported ?c) — no
      ;; (exists) inside the action body — so no witness params are
      ;; lifted and put-long-x keeps a single arity.
      (or
        (supported ?c2)
        (and (supported ?c1) (supported ?c3)))
    )
    :effect (and
  ...
  (:action put-long-y
    ...
    :precondition (and
      ...
      (empty ?c1) (empty ?c2) (empty ?c3)
      (puttable ?c1) (puttable ?c2) (puttable ?c3)
      ;; CENTRE c2 alone, OR both ENDS c1 and c3 (unary supported only).
      (or
        (supported ?c2)
        (and (supported ?c1) (supported ?c3)))
    )
