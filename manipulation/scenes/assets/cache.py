...
class AssetCache:
    ...
    The cache root defaults to ``~/.cache/manipulation/assets`` and can be
    overridden with the ``MANIPULATION_ASSETS_CACHE`` environment variable or
    ...
    def __init__(self, base_dir=None):
        self._base = Path(
            base_dir
            or os.environ.get(
                "MANIPULATION_ASSETS_CACHE",
                Path.home() / ".cache" / "manipulation" / "assets",
            )
        )
