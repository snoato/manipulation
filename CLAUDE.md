...
1. **Think before coding.** State assumptions explicitly before touching any file.
2. **Minimum viable, zero speculative features.** Implement only what is asked; no future-proofing or extras.
3. **Surgical changes.** Prefer the smallest diff that solves the problem; avoid unrelated edits.
4. **Define success, then loop until achieved.** Before starting, state the success criterion; verify it is met before declaring done.
5. **Request permissions ahead.** At the start of a multi-step task, identify and request all tool/file permissions needed so the work can proceed without interruptions.
...
python learn_stack_action.py           # SAC+HER stacking via TampandaGoalEnv (requires stable-baselines3)
python learn_stack_action.py --eval    # Evaluate saved model
```
...
**`tampanda.gym`** (`gym/`) — Gymnasium integration layer. `TampandaGymEnv` wraps any `ArmSceneBuilder` scene as a `gymnasium.Env` with configurable obs/action spaces and reward functions. `TampandaGoalEnv` adds HER-compatible goal structure (`object_pose` or `symbolic_predicates` mode). Wrappers: `PseudoGraspWrapper` (kinematic attachment on gripper close), `ExpertActionWrapper` (RRT*-based expert actions), `SymbolicRewardWrapper` (plan-progress shaping). `make_vec_env` creates spawn-safe parallel envs. `DomainBridge` integrates via `bridge_factory` — called fresh each `reset()` so parallel workers never share state.
